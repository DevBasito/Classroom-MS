...
--------------------------------------------------------
--------------------------------------------------------
--  DDL for Package Body OM_TAPIGEN
--------------------------------------------------------
...
IS    
   ...
   TYPE t_unique_constraint_info IS RECORD    
   (    
   ...
   IS    
      ...
      BEGIN    
         v_match_pos      :=    
            REGEXP_INSTR(g_tapi_code_blocks.template, v_pattern, v_start_pos);    
      END get_match_pos;    
      ...
      BEGIN    
         ...
         THEN    
            util_clob_append(g_tapi_code_blocks.dml_view_trigger    
...
CREATE OR REPLACE PACKAGE BODY #TABLE_NAME_26#_api IS    
  ...
  ----------------------------------------';    
   ...
   BEGIN    
      g_tapi_code_blocks.template := '     
  FUNCTION row_exists_yn( p_#PK_COLUMN_28# IN #TABLE_NAME#."#PK_COLUMN#"%TYPE )    
  RETURN VARCHAR2;    
  ----------------------------------------';    
      util_template_replace('API SPEC');    
      ----------------------------------------    
  ...
  IS    
  BEGIN    
    RETURN case when row_exists( p_#PK_COLUMN_28# => p_#PK_COLUMN_28# )     
             then ''Y''    
             else ''N''    
           end;    
  END;    
      ...
      THEN    
         ...
         LOOP    
            ...
            LOOP    
               ...
               THEN    
                  g_tab_unique_cons_column_info(j).column_name_28      :=    
                     SUBSTR(g_tab_unique_cons_column_info(j).column_name    
                          , 1    
                          , 28);    
  ...
  BEGIN    
    ...
    INSERT INTO #TABLE_NAME# ( #COLUMN_LIST_W_PK# )    
      VALUES ( v_pk, #PARAM_LIST_WO_PK# )'    
         || CASE WHEN g_sequence_name IS NOT NULL THEN ';' ELSE '    
      RETURN "#PK_COLUMN#" INTO v_pk;' END    
         || CASE WHEN g_enable_generic_change_log THEN q'[    
    create_change_log_entry( p_table     => '#TABLE_NAME#'    
                           , p_column    => '#PK_COLUMN#'    
                           , p_pk_id     => v_pk    
                           , p_old_value => 'ROW CREATED'    
                           , p_new_value => 'ROW CREATED' );]' ELSE NULL END    
         || '     
    RETURN v_pk;    
  END create_row;    
  ----------------------------------------';    
   ...
   BEGIN    
      ...
      THEN    
         ...
         LOOP    
            ...
            g_tab_substitutions_array('#PARAM_CALLING_LIST_UNIQUE#')      :=    
               LTRIM(g_tab_substitutions_array('#PARAM_CALLING_LIST_UNIQUE#')    
                   , c_list_delimiter);    
  ...
  BEGIN    
    ...
    RETURN read_row ( p_#PK_COLUMN_28# => v_pk );    
  END read_row;    
  ----------------------------------------';    
            util_template_replace('API BODY');    
         END LOOP;    
  ...
  IS    
    v_row   #TABLE_NAME#%ROWTYPE;'    
         || CASE WHEN g_enable_generic_change_log THEN '    
    v_count PLS_INTEGER := 0;' ELSE NULL END    
         || '    
  BEGIN    
    v_row := read_row( p_#PK_COLUMN_28# => p_#PK_COLUMN_28# );    
    IF v_row."#PK_COLUMN#" IS NOT NULL THEN    
  ...
  IS    
  BEGIN    
    DELETE FROM #TABLE_NAME# WHERE "#PK_COLUMN#" = p_#PK_COLUMN_28#;'    
         || CASE WHEN g_enable_generic_change_log THEN q'[    
    create_change_log_entry( p_table     => '#TABLE_NAME#'    
                           , p_column    => '#PK_COLUMN#'    
                           , p_pk_id     => p_#PK_COLUMN_28#    
                           , p_old_value => 'ROW DELETED'    
                           , p_new_value => 'ROW DELETED' );]' ELSE NULL END    
         || '    
  END delete_row;    
  ----------------------------------------';    
      util_template_replace('API BODY');    
   ...
   BEGIN    
      ...
      LOOP    
         ...
         THEN    
            ...
            g_tab_substitutions_array('#I_COLUMN_NAME_28#')      :=    
    ...
    IF v_row."#PK_COLUMN#" IS NOT NULL THEN    
      ...
      IF #I_COLUMN_COMPARE# THEN    
        UPDATE #TABLE_NAME#    
         ...
         WHERE "#PK_COLUMN#" = p_#PK_COLUMN_28#;'    
               || CASE WHEN g_enable_generic_change_log THEN q'[    
        create_change_log_entry( p_table     => '#TABLE_NAME#'    
                               , p_column    => '#I_COLUMN_NAME#'    
                               , p_pk_id     => p_#PK_COLUMN_28#    
                               , p_old_value => #I_OLD_VALUE#    
                               , p_new_value => #I_NEW_VALUE# );]' ELSE NULL END    
               || '    
      END IF;    
    END IF;    
  ...
  ELSIF DELETING THEN    
    '    
         || CASE    
               ...
               THEN    
                  '#TABLE_NAME_26#_api.delete_row( p_#PK_COLUMN_28# => :old."#PK_COLUMN#");'    
               ELSE    
      ...
      IS    
         ...
         BEGIN    
           ...
           AND g_tab_substitutions_array('#COLUMN_PREFIX#') IS NULL    
            ...
            g_tab_substitutions_array('#GENERATED_BY#')      :=    
               util_get_user_name;    
...
comment on column generic_change_log.gcl_timestamp is 'The time when the change occured']';    
            ...
            BEGIN    
               ...
               g_tab_substitutions_array('#COLUMN_LIST_W_PK#')      :=    
                  LTRIM(g_tab_substitutions_array('#COLUMN_LIST_W_PK#')    
                      , c_list_delimiter);    
               g_tab_substitutions_array('#MAP_NEW_TO_PARAM_W_PK#')      :=    
                  LTRIM(g_tab_substitutions_array('#MAP_NEW_TO_PARAM_W_PK#')    
                      , c_list_delimiter);    
               g_tab_substitutions_array('#MAP_PARAM_TO_PARAM_W_PK#')      :=    
                  LTRIM(g_tab_substitutions_array('#MAP_PARAM_TO_PARAM_W_PK#')    
                      , c_list_delimiter);    
               g_tab_substitutions_array('#MAP_ROWTYPE_COL_TO_PARAM_W_PK#')      :=    
                  LTRIM(    
                     g_tab_substitutions_array(    
                   ...
                   , c_list_delimiter);    
               g_tab_substitutions_array('#PARAM_DEFINITION_W_PK#')      :=    
                  LTRIM(g_tab_substitutions_array('#PARAM_DEFINITION_W_PK#')    
                      , c_list_delimiter);    
               g_tab_substitutions_array('#PARAM_IO_DEFINITION_W_PK#')      :=    
                  LTRIM(g_tab_substitutions_array('#PARAM_IO_DEFINITION_W_PK#')    
                      , c_list_delimiter);    
               g_tab_substitutions_array('#PARAM_LIST_WO_PK#')      :=    
   ...
   PROCEDURE compile_api(    
    ...
    , -- if true, the following params are ignored, if API package are already    
...
/
  ...
  GRANT EXECUTE ON "CHCERP_HR"."OM_TAPIGEN" TO "CHCERP";
